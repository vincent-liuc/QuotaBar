...
actor AppUpdater {
    ...
    func installAndRelaunch(_ release: UpdateRelease) async throws {
        ...
        try launchInstaller(stagedApp: stagedApp, currentApp: currentApp, expectedVersion: release.version)
    ...
    private func launchInstaller(stagedApp: URL, currentApp: URL, expectedVersion: String) throws {
        ...
        expected_version=\(Self.shellQuote(expectedVersion))
        log=\(Self.shellQuote(logURL.path))
        exec >> "$log" 2>&1
        for pid in $(/usr/bin/pgrep -f "^$current/Contents/MacOS/QuotaBar$" 2>/dev/null); do
          if [[ "$pid" != "$old_pid" ]]; then /bin/kill "$pid" 2>/dev/null || true; fi
        done
        for _ in {1..120}; do
          /usr/bin/pgrep -f "^$current/Contents/MacOS/QuotaBar$" >/dev/null 2>&1 || break
          sleep 0.25
        done
        if /usr/bin/pgrep -f "^$current/Contents/MacOS/QuotaBar$" >/dev/null 2>&1; then exit 1; fi
        ...
        if /bin/mv "$staged" "$current"; then
          ...
          if [[ "$installed_version" == "$expected_version" ]] && /usr/bin/pgrep -f "^$current/Contents/MacOS/QuotaBar$" >/dev/null; then
            /bin/rm -rf "$backup"
            /bin/rm -f "$0"
            exit 0
          fi
