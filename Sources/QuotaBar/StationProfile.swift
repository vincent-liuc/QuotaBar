...
enum StationKind: String, Codable, CaseIterable, Sendable {
    case sub2API
    case newAPI
    ...
    var displayName: String {
        ...
        case .newAPI: return "New API / One API"
        }
    }

    var defaultAPIPath: String {
        switch self {
        case .sub2API: return "/api/v1"
        case .newAPI: return "/api"
        }
    }
}
...
struct StationProfile: Codable, Equatable, Identifiable, Sendable {
    ...
    private enum CodingKeys: String, CodingKey {
        case id, name, kind, serviceURL, apiPath, timezone, subscriptionSelection
    ...
    init(from decoder: Decoder) throws {
        ...
        kind = try container.decodeIfPresent(StationKind.self, forKey: .kind) ?? .sub2API
    ...
    func validated() throws -> StationProfile {
        ...
        let endpoint = try StationEndpoint(serviceURL: serviceURL, apiPath: apiPath, defaultAPIPath: kind.defaultAPIPath)
...
struct StationEndpoint: Equatable, Sendable {
    ...
    init(
        serviceURL rawServiceURL: String,
        apiPath rawAPIPath: String,
        defaultAPIPath: String = StationProfile.defaultAPIPath
    ) throws {
        ...
        if path.isEmpty { path = defaultAPIPath }
