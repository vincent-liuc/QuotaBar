...
- API Key 按当日实际用量从高到低排列，每行显示两个；日历图标表示今日用量，悬停可查看含义及完整的长文本
- 在固定高度内滚动展示站点时区内最近两天的最新 50 条使用记录
- 订阅额度下方显示周重置倒计时：单订阅不显示名称，多订阅逐项显示名称与各自倒计时；到期早于下一次重置时明确显示到期倒计时
- 自动汇总账号下全部有效且未到期订阅的周额度、已用量和剩余额度，每日用量也分别合计；无需选择订阅，旧版的手动订阅配置不再限制汇总
- 多订阅汇总显示订阅数量，倒计时超过三项时可滚动查看；各订阅剩余额度分别扣除已用量后取非负值再求和
- 可为每个站点单独开启周期重置联动（默认关闭）：单订阅检测到服务器周周期起点更新后，将所有非 `inactive` API Key 的已用额度清零；到期前最后一个周期也会处理，失败的 Key 会在下次刷新时重试；多订阅汇总时暂停此联动，避免不同周期相互影响
- 升级至 1.11.26 后首次读取服务器周周期只建立新基线，不根据旧版倒计时记录追溯清零
- 可在设置中分别隐藏累计指标、订阅额度、每日用量、API Key 明细和使用记录
- 设置项实时生效，文本输入在失焦后保存
...
- `/api/v1/subscriptions?timezone=<站点时区>`：有效订阅的每日/每周已用额度、额度上限和重置窗口，自动汇总展示
...
- `/api/v1/usage/dashboard/models`：IMG 指标汇总从 2026-01-01 至今的 `gpt-image-2.5-sunburst` 与 `gpt-image-2` 请求次数
